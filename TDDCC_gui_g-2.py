...
# ---------------------------------------------------------------------------
# NOTE ON CROSS-PLATFORM COLORS:
# Plain tk.Button ignores bg/fg color on macOS because Aqua renders native
# buttons and refuses to be recolored. ttk.Button + a "clam" theme respects
# custom colors on macOS, Windows, and Linux alike. We register one ttk
# style per unique color the first time it's requested, then reuse it.
_BUTTON_STYLE_CACHE = set()


def _ensure_ttk_theme(widget):
    """Switch the whole app to the 'clam' theme once (safe to call repeatedly)."""
    style = ttk.Style(widget)
    if style.theme_use() != "clam":
        try:
            style.theme_use("clam")
        except tk.TclError:
            pass  # 'clam' should always be available, but fail safe
    return style


def _darken(hex_color, factor=0.85):
    """Return a slightly darker shade of hex_color, used for hover/press states."""
...
def create_modern_button(parent, text, bg_color, command):
    """
    Cross-platform colored button (macOS / Windows / Linux).
    Same call signature as before: create_modern_button(parent, text, bg_color, command)
    """
    style = _ensure_ttk_theme(parent)

    style_name = f"Modern_{bg_color.lstrip('#')}.TButton"
    if style_name not in _BUTTON_STYLE_CACHE:
        hover = _darken(bg_color, 0.88)
        pressed = _darken(bg_color, 0.75)
        style.configure(
            style_name,
            background=bg_color,
            foreground="white",
        ...
        _BUTTON_STYLE_CACHE.add(style_name)
...
class IntroPage:
    def __init__(self, root):
        ...
        self.root.configure(bg="#0b1c2d")

        frame = tk.Frame(root, bg="#0b1c2d")
        frame.place(relx=0.5, rely=0.5, anchor="center")

        tk.Label(
            frame, text="Dynamic Cross-Correlation Analysis",
            font=("Helvetica", 32, "bold"), fg="white", bg="#0b1c2d"
        ).pack(pady=20)

        tk.Label(
            frame, text="Explore correlated motions in biomolecular systems",
            font=("Helvetica", 16), fg="#9ecfff", bg="#0b1c2d"
        ).pack(pady=10)

        create_modern_button(
            frame, "Launch Analysis Dashboard", "#1f6fd6", self.launch
        ).pack(pady=40, ipadx=20, ipady=6)
...
class CorrelationGUI:
    ...
    def build_gui(self):
        ...
        f_dcc = tk.LabelFrame(
            self.left_panel,
            text=" 0. Compute DCC from Trajectory ",
        ...
        f_input = tk.LabelFrame(self.left_panel, text=" 1. Input Data ",
                                font=section_font, bg="#ffffff", pady=4, padx=10)
        ...
        f_res = tk.LabelFrame(self.left_panel, text=" 2. Residue Limits ",
                              font=section_font, bg="#ffffff", pady=4, padx=10)
        ...
        f_adv = tk.LabelFrame(self.left_panel, text=" 3. Correlation Filters ",
                              font=section_font, bg="#ffffff", pady=4, padx=10)
        ...
        # Actions
        create_modern_button(self.left_panel, "RUN ANALYSIS", "#27ae60",
                             self.run_analysis).pack(fill="x", pady=1)

        f_dash = tk.Frame(self.left_panel, bg="#ffffff")
        f_dash.pack(fill="x", pady=2)
        create_modern_button(f_dash, "View Correlation Dashboard", "#2980b9",
        ...
        create_modern_button(f_csv, "Download Combined CSV", "#16a085",
                             self.download_combined_csv).pack(fill="x", pady=1)
        create_modern_button(f_csv, "Download Positive CSV", "#16a085",
    ...
    def open_correlation_dashboard(self):
        ...
        im1 = ax1.imshow(corr.where(mask_pos), cmap="Reds", vmin=0, vmax=1,
        ...
        im2 = ax2.imshow(corr.where(mask_neg), cmap="Blues_r", vmin=-1, vmax=0,
                         origin="lower", extent=grid_extent, aspect="auto", picker=True)
        ...
        cmap_bwr  = mcolors.LinearSegmentedColormap.from_list(
            "bwr", ["blue", "white", "red"])
        im3 = ax3.imshow(combined.values, cmap=cmap_bwr, vmin=-1, vmax=1,
                         origin="lower", extent=grid_extent, aspect="auto", picker=True)
    ...
    def open_time_series_window(self, i, j, ts):
        ...
        def save_png():
            path = filedialog.asksaveasfilename(
                ...
                initialfile=f"timeseries_i{i}_j{j}.png",
        ...
        create_modern_button(btn_frame, "Save PNG", "#2980b9", save_png).pack(
            side=tk.LEFT, padx=5)
        create_modern_button(btn_frame, "Save CSV", "#27ae60", save_csv).pack(
            side=tk.LEFT, padx=5)
        tk.Label(btn_frame,
